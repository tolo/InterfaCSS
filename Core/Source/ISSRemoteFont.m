...
@interface ISSRemoteFont ()

@property (nonatomic, strong) ISSUpdatableValueObserver* observer;
...
- (instancetype) initWithRemoteFont:(ISSDownloadableResource*)remoteFont fontSize:(CGFloat)fontSize {
    if ( self = [super init] ) {
        ...
        __weak ISSRemoteFont* weakSelf = self;
        _observer = [_remoteFont addValueUpdateObserverWithBlock:^(NSNotification * _Nonnull note) {
            [weakSelf valueUpdated];
        }];
    }
    return self;
}
