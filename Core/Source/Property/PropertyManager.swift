...
open class PropertyManager {
  ...
  open func apply(_ propertyValue: PropertyValue, onTarget targetElement: ElementStyle, styleSheetScope scope: StyleSheetScope) {
    guard let element = targetElement.uiElement else { return }
    ...
    logger.trace("Applying property value \(propertyValue.rawValue ?? "") to '\(property.fqn)' in '\(targetElement)'")
    property.setValue(propertyValue, onTarget: element)
  }
}
