...
char* const ISSCGColorTypeId = @encode(CGColorRef);
char* const ISSCGRectTypeId = @encode(CGRect);
char* const ISSCGPointTypeId = @encode(CGPoint);
char* const ISSUIEdgeInsetsTypeId = @encode(UIEdgeInsets);
char* const ISSUIOffsetTypeId = @encode(UIOffset);
char* const ISSCGSizeTypeId = @encode(CGSize);
char* const ISSCGAffineTransformTypeId = @encode(CGAffineTransform);
...
+ (ISSRuntimeProperty*) runtimePropertyWithName:(NSString*)name inClass:(Class)clazz lowercasedNames:(BOOL)lowercasedNames {
  ...
  return [self runtimePropertiesForClass:clazz excludingRootClasses:@[UIResponder.class, NSObject.class] lowercasedNames:lowercasedNames][effectiveName];
}

+ (NSDictionary*) runtimePropertiesForClass:(Class)clazz lowercasedNames:(BOOL)lowercasedNames {
  return [self runtimePropertiesForClass:clazz excludingRootClasses:@[UIResponder.class, NSObject.class] lowercasedNames:lowercasedNames];
}

+ (NSDictionary*) runtimePropertiesForClass:(Class)clazz excludingRootClasses:(NSArray*)rootClasses lowercasedNames:(BOOL)lowercasedNames {
...
+ (Class) classWithName:(NSString*)className {
  ...
  if( !clazz ) { // If direct match not found, check if it's a Swift class name
    //NSArray<NSBundle*>* bundles = NSBundle.allBundles + @[[NSBundle mainBundle]];
    NSArray<NSBundle*>* bundles = [@[[NSBundle mainBundle]] arrayByAddingObjectsFromArray:NSBundle.allBundles];
    for(NSBundle* bundle in bundles) {
      NSString* appName = [[bundle objectForInfoDictionaryKey:(NSString*)kCFBundleNameKey] stringByReplacingOccurrencesOfString:@" " withString:@"_"];
