...
- (ISSParser*) many {
    return [self many:0 onlyValid:NO flatten:NO];
}

- (ISSParser*) many1 {
    ISSParser* many = [self many:1 onlyValid:NO flatten:NO];
    many.name = @"many";
    return many;
}

- (ISSParser*) manyActualValues {
    ISSParser* many = [self many:0 onlyValid:YES flatten:NO];
    many.name = @"manyActualValues";
    return many;
}

- (ISSParser*) manyActualValuesFlat {
    ISSParser* many = [self many:0 onlyValid:YES flatten:YES];
    many.name = @"manyActualValuesFlat";
    return many;
}

- (ISSParser*) many1ActualValues {
    ISSParser* many = [self many:1 onlyValid:YES flatten:NO];
    many.name = @"many1ActualValues";
    return many;
}

- (ISSParser*) many1ActualValuesFlat {
    ISSParser* many = [self many:1 onlyValid:YES flatten:YES];
    many.name = @"many1ActualValuesFlat";
    return many;
}

- (ISSParser*) many:(NSUInteger)minCount onlyValid:(BOOL)onlyValid flatten:(BOOL)flatten {
    return [ISSParser parserWithBlock:^id (NSString* input, ISSParserStatus* status) {
        ...
        do {
            ...
            if( status->match ) {
                if( !onlyValid || value != [NSNull null] ) {
                    if( flatten && [value isKindOfClass:NSArray.class] ) {
                        [values addObjectsFromArray:[value iss_flattened]];
                    } else {
