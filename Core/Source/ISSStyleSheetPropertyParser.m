...
- (NSArray*) basicColorValueParsers {
    ISSParser* rgb = [[self.styleSheetParser parameterStringWithPrefix:@"rgb"] transform:^id(NSArray* cc) {
    ...
    ISSParser* rgba = [[self.styleSheetParser parameterStringWithPrefix:@"rgba"] transform:^id(NSArray* cc) {
        UIColor* color = [UIColor magentaColor];
        if( cc.count == 4 ) {
...
- (ISSParser*) colorFunctionParser:(NSArray*)colorValueParsers preDefColorParser:(ISSParser*)preDefColorParser {
    ...
    ISSParser* colorFunctionParser = [[ISSParser sequential:@[self.styleSheetParser.identifier, [ISSParser unichar:'(' skipSpaces:YES],
                                                              [ISSParser choice:colorValueParsers], [ISSParser unichar:',' skipSpaces:YES], self.styleSheetParser.anyName, [ISSParser unichar:')' skipSpaces:YES]]] transform:^id(id value) {
        UIColor* color = [UIColor magentaColor];
...
- (NSArray*) colorCatchAllParser:(ISSParser*)imageParser {
    ...
    ISSParser* catchAll = [self.styleSheetParser.anyName transform:^id(id value) {
...
- (ISSParser*) imageParsers:(ISSParser*)imageParser colorValueParsers:(NSArray*)colorValueParsers {
    ISSParser* preDefColorParser = [self.styleSheetParser.identifier transform:^id(id value) {
    ...
    ISSParser* urlImageParser = [[self.styleSheetParser parameterStringWithPrefix:@"url"] transform:^id(NSArray* parameters) {
    ...
    ISSParser* catchAll = [self.styleSheetParser.anythingButControlChars transform:^id(id value) {
...
- (ISSParser*) simpleNumericParameterStringWithPrefixes:(NSArray*)prefixes optionalPrefix:(BOOL)optionalPrefix {
    ISSParser* parameterStringWithPrefixParser = [[self.styleSheetParser parameterStringWithPrefixes:prefixes] transform:^id(NSArray* parameters) {
