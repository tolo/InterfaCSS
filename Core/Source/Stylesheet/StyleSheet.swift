...
public typealias RefreshableStyleSheetFileModifiecationObserver = (RefreshableStyleSheet) -> Void
public typealias StyleSheetRefeshCompletion = (RefreshableStyleSheet, Bool) -> Void
...
public class RefreshableStyleSheet: StyleSheet {
  ...
  required init(styleSheetURL: URL, name: String?, group groupName: String?, content: StyleSheetContent = .empty) {
    if styleSheetURL.isFileURL {
      ...
      refreshableResource = RefreshableRemoteResource(withURL: styleSheetURL)
    }
  ...
  open func startMonitoringStyleSheetModification(_ modificationObserver: @escaping RefreshableStyleSheetFileModifiecationObserver) {
  ...
  open func refreshStylesheet(with styleSheetManager: StyleSheetManager, andCompletionHandler completionHandler: @escaping StyleSheetRefeshCompletion, force: Bool) {
    refreshableResource.refresh(intervalDuringError: styleSheetManager.stylesheetAutoRefreshInterval, force: force) { success, responseString, error in
      if success, let responseString = responseString {
        ...
        if let styleSheetContent = styleSheetContent {
          ...
          debug(.stylesheets, "Refreshable stylesheet didn't contain any rulesets!")
