...
//  }
  ...
  public init(uiElement: AnyObject) {
    ...
    markDirtyNotificationToken = iCSS.markCachedStylingInformationAsDirtyNotification.observe { [weak self] in
  ...
  public class func markAllCachedStylingInformationAsDirty() {
    iCSS.markCachedStylingInformationAsDirtyNotification.post()
  }
  ...
  public func applyStyling(with styler: Styler = StylingManager.shared, includeSubViews: Bool = true, force: Bool = false) {
    styler.applyStyling(self, includeSubViews: includeSubViews, force: force)
  }
}
