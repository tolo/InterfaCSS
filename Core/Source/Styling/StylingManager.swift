...
public class StylingManager: Styler { // TODO: Does it need to be NSObject?
  ...
  required init(propertyManager: PropertyManager? = nil, styleSheetManager: StyleSheetManager? = nil) {
    ...
    iCSS.shouldClearAllCachedStylingInformation.observe { [weak self] _ in
  ...
  func style(for uiElement: Stylable, resetIfNeeded: Bool = true) -> ElementStyle {
    let element = uiElement.interfaCSS
    if element.isDirty && resetIfNeeded {
  ...
  public func applyStyling(_ stylable: Stylable, includeSubViews: Bool = true, force: Bool = false) { // From protocol Styler
  ...
  public func applyStyling(_ element: ElementStyle, includeSubViews: Bool = true, force: Bool = false) { // From protocol Styler
    applyStyling(element, includeSubViews: includeSubViews, force: force, styleSheetScope: nil, styler: nil)
  }
  
  
  // MARK: - Main styling methods
  
  /// Applies styling of the specified UI object and optionally also all its children.
  public func applyStyling(_ element: ElementStyle, includeSubViews: Bool = true, force: Bool = false, styleSheetScope: StyleSheetScope? = nil, styler: Styler? = nil) {
