...
class InterfaCSSTests: XCTestCase {
  ...
  func testLabelTextAlign() {
    let styler = initializeWithStyleSheet("stylingTest-properties")
    
    let rootView = UIView()
    rootView.interfaCSS.addStyleClass("labelTest")
    let label = UILabel()
    rootView.addSubview(label)
    label.interfaCSS.styleClass = "test1"
    styler.applyStyling(rootView)
    
    XCTAssertEqual(label.text, "test")
    XCTAssertEqual(label.textAlignment, .center)
    
    label.interfaCSS.styleClass = "test2"
    styler.applyStyling(label)
    XCTAssertEqual(label.textAlignment, .left)
    
    label.interfaCSS.styleClass = "test3"
    styler.applyStyling(label)
  ...
  func testCompoundFontProperty() {
    ...
    // Note: this test will only be meaningful when non-standard font size is set in simulator
    label.interfaCSS.addStyleClass("cssfont3")
    styler.applyStyling(label)
    XCTAssertEqual(label.font.pointSize, UIFont.systemFont(ofSize: 42).scaledFont(for: .largeTitle).pointSize)
  }
