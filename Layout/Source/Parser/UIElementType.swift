...
public enum UIElementType: String, StringParsableEnum {
  ...
  static func typeFrom(elementName _elementName: String, parentViewNode: AbstractViewTreeNode?) -> UIElementType {
    ...
    if let value = enumValue(from: elementName) {
      ...
      switch elementName {
        ...
        case "cell":
          if parentViewNode?.elementType == .tableView { return .tableViewCell }
          else if parentViewNode?.elementType == .collectionView { return .collectionViewCell }
          else { return .view }
        default: return .view
      }
    }
