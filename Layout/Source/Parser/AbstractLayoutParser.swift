...
extension AbstractViewTreeParser: XMLParserDelegate {
  ...
  public func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String: String] = [:]) {
    ...
    Logger.layout.trace("Parsing element \(elementName)")
    ...
    if elementName ^= AbstractViewTreeParser.rootElementName {
      ...
      if let parentNode = nodeStack.last, case .viewNode(let _parentViewNode) = parentNode {
        parentViewNode = _parentViewNode
      }
      
      // Attributes
      let attributes = ElementAttributes(forElementName: elementName, attributes: attributeDict, withStyler: styler)
      let elementType = UIElementType.typeFrom(elementName: elementName, parentViewNode: parentViewNode)
      
      let viewTreeNode = AbstractViewTreeNode(elementType: elementType, attributes: attributes)
      if let parentViewNode = parentViewNode {
