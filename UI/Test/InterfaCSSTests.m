...
- (void) testElementStyleCaching {
    ...
    ISSElementStylingProxy* details = [[InterfaCSS interfaCSS] stylingProxyFor:label];
    XCTAssertFalse(details.stylesCacheable, @"Expected styles to be NOT cacheable");
    
    UIWindow* window = [[UIWindow alloc] init];
    [window addSubview:view];
    
    details = [[InterfaCSS interfaCSS] stylingProxyFor:label];
...
- (void) testElementStyleCachingUsingCustomStylingIdentity {
    ...
    ISSElementStylingProxy* details = [[InterfaCSS interfaCSS] stylingProxyFor:view];
    XCTAssertTrue(details.stylesCacheable, @"Expected styles to be cacheable");
}
...
- (void) testNestedPropertiesOfSameTypeUsesDifferentStyleIdentities {
    ...
    ISSElementStylingProxy* textLabelDetails = [[InterfaCSS interfaCSS] stylingProxyFor:cell.textLabel];
    ISSElementStylingProxy* detailTextLabelDetails = [[InterfaCSS interfaCSS] stylingProxyFor:cell.detailTextLabel];
...
- (void) testIntrospection {
    ...
    //NSValue* val = [NSValue value:&r withObjCType:@encode(UIViewAutoresizing)];
    
    NSDictionary* leMupp = [ISSRuntimeIntrospectionUtils runtimePropertiesForClass:UILabel.class excludingRootClass:[UILabel.class superclass] lowercasedNames:NO];
    NSLog(@"Properties UILabel: %@", leMupp);
    
    leMupp = [ISSRuntimeIntrospectionUtils runtimePropertiesForClass:UIView.class excludingRootClass:[UIView.class superclass] lowercasedNames:NO];
    NSLog(@"Properties UIView: %@", leMupp);
    
    leMupp = [ISSRuntimeIntrospectionUtils runtimePropertiesForClass:CALayer.class excludingRootClass:[CALayer.class superclass] lowercasedNames:NO];
    NSLog(@"Properties CALayer: %@", leMupp);
    
    UIViewAutoresizing autoresizingValue = UIViewAutoresizingFlexibleWidth;
    BOOL result = [ISSRuntimeIntrospectionUtils invokeSetterForProperty:@"autoresizingMask" ignoringCase:NO withValue:@(autoresizingValue) inObject:view];
    XCTAssertTrue(result);
    XCTAssertEqual(view.autoresizingMask, autoresizingValue);
    
    CGRect rect = CGRectMake(1, 2, 3, 4);
    result = [ISSRuntimeIntrospectionUtils invokeSetterForProperty:@"frame" ignoringCase:NO withValue:[NSValue valueWithCGRect:rect] inObject:view];
    XCTAssertTrue(result);
    XCTAssertEqualObjects(NSStringFromCGRect(view.frame), NSStringFromCGRect(rect));
    
    NSInteger tag = 42;
    result = [ISSRuntimeIntrospectionUtils invokeSetterForProperty:@"tag" ignoringCase:NO withValue:@(tag) inObject:view];
    XCTAssertTrue(result);
    XCTAssertEqual(view.tag, tag);
    
    CGFloat alpha = 0.5;
    result = [ISSRuntimeIntrospectionUtils invokeSetterForProperty:@"alpha" ignoringCase:NO withValue:@(alpha) inObject:view];
    XCTAssertTrue(result);
    ISSAssertEqualFloats(view.alpha, alpha);
    
    id returnValue = [ISSRuntimeIntrospectionUtils invokeGetterForProperty:@"alpha" ignoringCase:NO inObject:view];
    ISSAssertEqualFloats([returnValue floatValue], alpha);
    
    CGFloat cornerRadius = 3;
    result = [ISSRuntimeIntrospectionUtils invokeSetterForKeyPath:@"layer.cornerRadius" ignoringCase:NO withValue:@(cornerRadius) inObject:view];
    XCTAssertTrue(result);
    ISSAssertEqualFloats(view.layer.cornerRadius, cornerRadius);
    
    
    //NSValue* val = [NSValue value:&r withObjCType:@encode(UIViewAutoresizing)];
    
    /*NSInvocation* inv = [ISSRuntimeIntrospectionUtils findSetterForProperty:@"frame" ignoringCase:NO inObject:view];
    if (inv != nil) {
        const char* argType = [inv.methodSignature getArgumentTypeAtIndex:2];
        NSString* argTypeString = [[NSString alloc] initWithCString:argType encoding:NSUTF8StringEncoding];
        NSLog(@"Type: %@", argTypeString);
